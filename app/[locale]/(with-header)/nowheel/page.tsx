...
export default function NoWheelPage() {
  const t = useTranslations('NoWheel');
  const wheelT = useTranslations('NoWheel.Wheel');

  const noWheelItems: WheelSliceItem[] = useMemo(() => [
    {
  ...
  const handleSpinResult = useCallback((result: WheelSliceItem) => {
    console.log('NoWheel spin result:', result);
  }, []);

  return (
    <div className='relative w-full bg-gradient-to-b from-red-50 to-white'>
      <div className='relative mx-auto w-full max-w-7xl flex-1 px-4 sm:px-6 lg:px-8'>
        ...
        <section className='rounded-2xl py-12'>
          <div className='mb-6 flex flex-col items-center text-center sm:mb-8 lg:mx-auto'>
            <h1 className='mb-3 text-2xl font-extrabold bg-gradient-to-r from-red-600 to-rose-600 bg-clip-text text-transparent sm:mb-4 sm:text-3xl md:text-4xl lg:text-6xl animate-hardware'>
              {t('title')}
            </h1>
          ...
          <YesNoWheel customItems={noWheelItems} sliceRepeats={3} onSpinComplete={handleSpinResult} />
        </section>

        {/* Features Section */}
        <section className='rounded-2xl bg-white p-8 shadow-xl backdrop-blur-sm bg-white/80 border border-slate-100'>
          <h2 id='features' className='mb-8 pb-3 text-2xl font-bold bg-gradient-to-r from-red-600 to-rose-600 bg-clip-text text-transparent'
          ...
          <div className='grid gap-6 md:grid-cols-2'>
            {t.raw('Features.items').map((item: { title: string; description: string }) => (
              ...
              >
                <h3 className='mb-3 text-xl font-semibold bg-gradient-to-r from-red-600 to-rose-600 bg-clip-text text-transparent'>{item.title}</h3>
          ...
          {/* Welcome Section */}
          <h2 id='welcome' className='mb-8 mt-16 pb-3 text-2xl font-bold bg-gradient-to-r from-red-600 to-rose-600 bg-clip-text text-transparent'>
            {t('Welcome.title')}
          </h2>
          ...
          {/* What Is Section */}
          <h2 id='introduction' className='mb-8 mt-16 pb-3 text-2xl font-bold bg-gradient-to-r from-red-600 to-rose-600 bg-clip-text text-transparent'>
            {t('WhatIs.title')}
          </h2>
          <div className='rounded-xl bg-white p-6 shadow-sm border border-slate-100 transition-all duration-300 hover:shadow-md'>
          ...
          <h2
            id='how-to-use'
            className='mb-8 mt-16 pb-3 text-2xl font-bold bg-gradient-to-r from-red-600 to-rose-600 bg-clip-text text-transparent'
          >
            {t('HowToUse.title')}
          </h2>
          <div className='grid gap-6 md:grid-cols-2'>
            {t.raw('HowToUse.steps').map((item: { step: string; description: string }, index: number) => (
              ...
              >
                <div className="flex items-center gap-3 mb-3">
                  <span className="flex items-center justify-center w-8 h-8 rounded-full bg-gradient-to-r from-red-600 to-rose-600 text-white font-bold text-sm">
                    {index + 1}
                  </span>
                  <h3 className='text-xl font-semibold bg-gradient-to-r from-red-600 to-rose-600 bg-clip-text text-transparent'>{item.step}</h3>
                </div>
                <p className='leading-relaxed text-slate-700'>{item.description}</p>
              </div>
            ))}
          ...
          <div className='grid gap-6 md:grid-cols-2'>
            {t.raw('Benefits.items').map((item: { title: string; description: string }) => (
              <div
                key={`benefit-${item.title.toLowerCase().replace(/\s+/g, '-')}`}
                className='rounded-xl bg-white p-6 shadow-sm border border-slate-100 transition-all duration-300 hover:shadow-md hover:-translate-y-1'
              >
                <h3 className='mb-3 text-xl font-semibold bg-gradient-to-r from-red-600 to-rose-600 bg-clip-text text-transparent'>{item.title}</h3>
                <p className='leading-relaxed text-slate-700'>{item.description}</p>
          ...
          {/* FAQ Section */}
          <h2 id='faq' className='mb-8 mt-16 pb-3 text-2xl font-bold bg-gradient-to-r from-red-600 to-rose-600 bg-clip-text text-transparent'>
            {t('Faq.title')}
          </h2>
              ...
              >
                <summary className='flex cursor-pointer list-none items-center justify-between font-medium'>
                  <div className="flex items-center gap-3">
                    <span className="flex-shrink-0 w-6 h-6 flex items-center justify-center rounded-full bg-gradient-to-r from-red-600 to-rose-600 text-white font-bold text-xs">
                      Q
                    </span>
